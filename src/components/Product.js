...
function Product() {
  ...
  const fetchProducts = async (page, sort, brands) => {
    ...
    if (sort === "asc") {
      ...
      const response = await fetch(
        `http://13.200.154.116:8081/product/api/products?page=${page}&size=${itemsPerPage}&sort=${sort}${brandQuery}`
      );
      if (!response.ok) {
  ...
  const fetchBrands = async () => {
    try {
      const response = await fetch("http://13.200.154.116:8081/product/brands");
      if (!response.ok) {
